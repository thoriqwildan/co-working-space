...
export class BookingService {
  ...
  async create(
    ...
    const spaceMustExist = await this.prismaService.space.findFirst({
      where: { space_id: Number(createBookingDto.space_id) },
    });
    if (!spaceMustExist) {
      throw new HttpException('Space not Found', 404);
    }
    ...
    if (spaceMustBeNotAvailable) {
      throw new HttpException('Space is booked', 400);
    }
  ...
  async update(
    ...
    const data = await this.prismaService.booking.findFirst({
    ...
    if (updateBookingDto.user_id) {
      data.user_id = updateBookingDto.user_id;
    }
    if (updateBookingDto.space_id) {
      data.space_id = updateBookingDto.space_id;
    }
    if (updateBookingDto.start_time) {
      data.start_time = new Date(updateBookingDto.start_time);
    }
    if (updateBookingDto.end_time) {
      data.end_time = new Date(updateBookingDto.end_time);
    }
