...
export class SpacesService {
  ...
  async update(
    ...
    const data = await this.prismaService.space.findFirst({
    ...
    if (updateSpaceDto.location) {
      data.location = updateSpaceDto.location;
    }
    if (updateSpaceDto.capacity) {
      data.capacity = updateSpaceDto.capacity;
    }
    if (updateSpaceDto.type) {
      data.type = updateSpaceDto.type;
    }
    if (updateSpaceDto.price) {
      data.price = updateSpaceDto.price;
    }
    if (updateSpaceDto.equipment && roomdata) {
      roomdata.equiment = updateSpaceDto.equipment;
    }
